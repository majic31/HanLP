...
public abstract class AbstractDataSet implements IDataSet {
    ...
    public IDataSet load(String folderPath, String charsetName, double percentage) throws IllegalArgumentException, IOException {
        ...
        if (file.isDirectory()) {
            return loadDir(folderPath, charsetName, percentage);
        } else if (file.isFile()) {
            return loadFile(file, charsetName, percentage);
        } else {
            return null;
        }
    }
    ...
    public IDataSet loadFile(File file, String charsetName, double percentage) throws IllegalArgumentException, IOException {
        if (!file.exists()) {
            throw new IllegalArgumentException(String.format("文件 %s 不存在", file.getAbsolutePath()));
        }
        if (percentage > 1.0 || percentage < -1.0) throw new IllegalArgumentException("percentage 的绝对值必须介于[0, 1]之间");
        logger.start("模式:%s\n文本编码:%s\n文件:%s\n加载中...\n", testingDataSet ? "测试集" : "训练集", charsetName, file.getAbsolutePath());
        ...
        while ((line = bufferReader.readLine()) != null) {
            ...
            String data = arr[1];
            List<String> list = dataMap.get(tagName);
            if(list == null) {
                list = new ArrayList<>();
        ...
        for (Map.Entry<String, List<String>> entry : dataMap.entrySet()) {
            ...
            if (percentage > 0) {
                b = 0;
                e = (int) (length * percentage);
            } else {
                b = (int) (length * (1 + percentage));
                e = length;
            }
            ...
            for (int i = b; i < e; i++) {
                add(key, list.get(i));
                if (i % logEvery == 0) {
                    logger.out("%c[%s]...%.2f%%", 13, key, MathUtility.percentage(i - b + 1, e - b));
                }
            }
            logger.out(" %d 行记录\n", e - b);
        }
    ...
    public IDataSet loadDir(String folderPath, String charsetName, double percentage) throws IllegalArgumentException, IOException {
